...
# Modifications copyright (C) 2020 Matilde Gonçalves
...
def lowest_cost_action(ic, dc, sc, im, dm, sm, cost):
	...
	* sm - substitution match (score)
	...
	if min_cost == sc and cost == 0:
		best_action = EQUAL
		best_match_count = sm
	elif (min_cost == sc and cost == 1) or (min_cost == sc and cost == 2):
		best_action = REPLACE
		best_match_count = sm
...
def edit_distance(seq1, seq2, action_function=lowest_cost_action, test=operator.eq):
	...
	for i in range(1, m + 1):
		...
		for j in range(1, n + 1):
			...
			if seq1[i - 1] in tipo and seq2[j - 1] in tipo and not test(seq1[i - 1], seq2[j - 1]):

				cost = 2
			elif seq1[i - 1] in tipo and seq2[j - 1] in tipo and test(seq1[i - 1], seq2[j - 1]):
				cost = 0

			elif seq1[i - 1] not in tipo and seq2[j - 1] not in tipo and not test(seq1[i - 1], seq2[j - 1]):
				cost = 1

			elif seq1[i - 1] not in tipo and seq2[j - 1] not in tipo and test(seq1[i - 1], seq2[j - 1]):
				cost = 0
	...
	for i in range(1, m + 1):
		...
		for j in range(1, n + 1):
			...
			if cost == 0:
				...
				ins_cost = d1[j - 1] + cost  # insertion
				del_cost = d0[j] + cost  # deletion
